...
def test_metadata_file_skips_a_bad_stream(tmp_path, caplog):
    fname = tmp_path / "m.h5"
    with caplog.at_level(logging.ERROR):
        io.write_metadata_hdf5(
            fname, {"good": [{"x": 1}], "bad": object()}
        )
...
def test_s11_autogenerated_name_encodes_the_mode(tmp_path):
    ...
    assert any(
        p.name.startswith("recs11_") for p in tmp_path.glob("*.h5")
    )
...
def test_s11_autogenerated_names_do_not_collide(tmp_path):
    ...
    for _ in range(3):
        io.write_s11_file(
            {"ant": np.array([1 + 0j])}, {}, save_dir=tmp_path
        )
...
def test_corr_header_bool_is_not_an_int(corr_header):
    ...
    assert any(
        "acc_bins" in v for v in io.validate_corr_header(corr_header)
    )
...
def test_corr_header_survives_a_round_trip_through_hdf5(
    tmp_path, corr_header
):
...
def test_sensor_schema_validation_accepts_a_good_entry():
    assert io.validate_metadata(
        sensor_entry("lidar"), io.SENSOR_SCHEMAS["lidar"]
    ) == []
...
def test_schemaless_reduction_sniffs_types():
    entries = [
        {"sensor_name": "m", "status": "update", "f": 1.0, "b": False,
         "i": 4, "s": "a"},
        {"sensor_name": "m", "status": "update", "f": 3.0, "b": True,
         "i": 2, "s": "b"},
    ]
    avg = io.avg_metadata(entries)
...
def test_eig_header_augmented_with_computed_fields(
    ...
    assert header["dfreq"] == pytest.approx(
        500e6 / (2 * eig_header["nchan"])
    )
    assert header["inttime"] == pytest.approx(
        2**20 * 2 / 500e6
    )
    assert np.allclose(
        header["times"],
        eig_header["sync_time"]
        + np.arange(3) * header["inttime"],
    )
...
def test_eig_header_pack_unpack_round_trip(eig_header):
    ...
    assert np.array_equal(
        unpacked["acc_cnt"], np.array(eig_header["acc_cnt"])
    )
...
def test_read_hdf5_returns_plain_numpy(tmp_path, corr_header):
    ...
    fname = tmp_path / "f.h5"
    io.write_hdf5(
        fname, {"0": np.zeros((2, 4), dtype=np.int32)}, corr_header
    )
